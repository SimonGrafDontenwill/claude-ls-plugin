...
function candidatePaths(): string[] {
  ...
  if (isWindows) {
    ...
    candidates.push(
      join(localAppData, "opencode", "bin", "codeblock-ls", "codeblock-language-server.exe"),
    );
  } else {
    const xdgData = process.env["XDG_DATA_HOME"] ?? join(homedir(), ".local", "share");
    candidates.push(
      join(xdgData, "opencode", "bin", "codeblock-ls", "codeblock-language-server"),
    );
  }
...
function findJarCandidates(): string[] {
  const envJar = process.env["CODEBLOCK_LS_JAR"];
  if (envJar) return [envJar];
...
export function launchLanguageServer(): LaunchResult {
